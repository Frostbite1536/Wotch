...
// Agent Runtime v2 lives in testable CommonJS modules. main.js only supplies
// Electron lifecycle services and application-specific dependencies.
const { createAgentRuntimeV2 } = require("./agent");
const agentRuntimeV2 = createAgentRuntimeV2({
  settingsDir: SETTINGS_DIR,
  bundledAgentsDir: BUILTIN_AGENTS_DIR,
  pty,
  safeStorage,
  getSettings: () => settings,
  updateAgentSettings: (patch) => {
    const current = { ...(settings.agentSettings || DEFAULT_SETTINGS.agentSettings) };
    if (Object.prototype.hasOwnProperty.call(patch, "enabled")) current.enabled = Boolean(patch.enabled);
    if (Object.prototype.hasOwnProperty.call(patch, "autoTriggerEnabled")) current.autoTriggerEnabled = Boolean(patch.autoTriggerEnabled);
    if (Object.prototype.hasOwnProperty.call(patch, "maxConcurrentAgents")) {
      const value = Number(patch.maxConcurrentAgents);
      if (!Number.isInteger(value) || value < 1 || value > 10) throw new Error("maxConcurrentAgents must be 1-10");
      current.maxConcurrentAgents = value;
    }
    if (Object.prototype.hasOwnProperty.call(patch, "defaultApprovalMode")) {
      if (!["suggest-only", "ask-first", "auto-execute"].includes(patch.defaultApprovalMode)) throw new Error("Invalid defaultApprovalMode");
      current.defaultApprovalMode = patch.defaultApprovalMode;
    }
    settings.agentSettings = current;
    saveSettings(settings);
    return { ...current };
  },
  getCredential: () => credentialManager.getKey(),
  getMainWindow: () => mainWindow,
  getKnownProjects: () => [...knownProjectPaths],
  isProjectAllowed: (projectPath) => isKnownProjectPath(projectPath),
  getStatus: () => claudeStatus.getAggregateStatus(),
  gitCheckpoint,
  readTerminal: async (lines = 200) => {
    if (!mainWindow || mainWindow.isDestroyed()) return { content: "(window not available)" };
    return new Promise((resolve) => {
      const timeout = setTimeout(() => resolve({ content: "(timeout reading buffer)" }), 5000);
      const handler = (_event, data) => {
        clearTimeout(timeout);
        ipcMain.removeListener("terminal-buffer-response", handler);
        resolve({ content: typeof data === "string" ? data : data?.buffer || "(empty)" });
      };
      ipcMain.on("terminal-buffer-response", handler);
      mainWindow.webContents.send("terminal-buffer-read", { lines });
    });
  },
  notify: (payload) => {
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send("plugin-notification", { pluginId: "agent", message: payload.message, type: payload.type || "info", duration: 3000 });
    }
  },
});
const agentManager = agentRuntimeV2.manager;
...
function isKnownProjectPath(p) {
  ...
  return [...knownProjectPaths].some((known) => normalize(known) === candidate);
}
...
// ── Agent Runtime v2 IPC (including one-release compatibility wrappers) ──
agentRuntimeV2.registerIpc(ipcMain);
...
ipcMain.handle("claude-set-api-key", async (_event, { apiKey }) => {
  try {
    ...
    agentManager.redactor.addSecret(apiKey);
